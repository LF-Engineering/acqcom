...
func acqcom(db *sqlx.DB, when time.Time, oid, nid int) {
	...
	for rows.Next() {
		...
		if cnt > 0 {
			fmt.Printf("enrollment %d needs manual update, it intersects %v but also has new company's enrollments 'select * from enrollments where uuid = '%s' and organization_id in (%d, %d) and project_slug = '%s' order by start, end;'\n", id, when, uuid, oid, nid, pSlug)
			continue
		}
